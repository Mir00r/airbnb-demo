...
interface HouseholdRepository : JpaRepository<Household, Long> {
    ...
    @Query("SELECT hh FROM Household hh WHERE (:q IS NULL OR LOWER(hh.createdBy) LIKE %:q% OR LOWER(hh.hostName) LIKE %:q%) AND (:propertyType IS NULL OR hh.propertyType=:propertyType) AND (:rentType IS NULL OR hh.rentType=:rentType) AND (:status IS NULL OR hh.status=:status) AND (:available IS NULL OR hh.available=:available) AND (:bed IS NULL OR hh.numberOfBed=:bed) AND (:bath IS NULL OR hh.numberOfBath=:bath) AND (:balcony IS NULL OR hh.numberOfBalcony=:balcony) AND (:size IS NULL OR hh.size=:size) AND (:price IS NULL OR hh.rentPrice=:price) AND hh.deleted=FALSE")
    fun search(
