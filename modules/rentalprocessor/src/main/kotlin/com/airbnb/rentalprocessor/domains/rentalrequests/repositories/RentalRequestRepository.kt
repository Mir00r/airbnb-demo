...
interface RentalRequestRepository : JpaRepository<RentalRequest, Long> {
    ...
    @Query("SELECT rr FROM RentalRequest rr WHERE (:q IS NULL OR LOWER(rr.createdBy) LIKE %:q%) AND (:status IS NULL OR rr.status=:status) AND (:requestedById IS NULL OR rr.requestedBy.id=:requestedById) AND (:requestedToId IS NULL OR rr.requestedTo.id=:requestedToId) AND (:cancelledById IS NULL OR rr.cancelledBy.id=:cancelledById) AND (:assignedToId IS NULL OR rr.assignedTo.id=:assignedToId) AND (:householdId IS NULL OR rr.household.id=:householdId) AND (rr.checkIn=:checkIn) AND (rr.checkOut=:checkOut) AND rr.deleted=FALSE")
    fun search(
